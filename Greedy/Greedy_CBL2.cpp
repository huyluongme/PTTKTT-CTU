...
void print_data(std::vector<DoVat> dsdv, float W) {
	...
	for (i = 0; i < dsdv.size(); i++) {
		printf("|%2d |%-18s|%9.2f|%9.2f|%10d|%9.2f|%6d     |\n",
			i + 1, dsdv[i].TenDV.c_str(), dsdv[i].TL, dsdv[i].GT, dsdv[i].SL, dsdv[i].DG, dsdv[i].PA);
...
void Greedy(std::vector<DoVat>* dsdv, float W) {
	for (int i = 0; i < dsdv->size(); i++) {
		dsdv->at(i).PA = std::min((int)(W / dsdv->at(i).TL), dsdv->at(i).SL);
